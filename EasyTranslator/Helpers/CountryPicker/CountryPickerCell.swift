...
public final class CountryPickerCell: UITableViewCell {
    ...
    func set(language: SupportedLanguage) {
        accessibilityIdentifier = language.isoCode
        
        let paragraphStyle = NSMutableParagraphStyle()

        paragraphStyle.lineSpacing = 3

        let attributedText = NSMutableAttributedString(string: "\(language.name)\n",
                                                       attributes: [NSAttributedString.Key.font: CountryManager.shared.config.countryNameTextFont,
                                                                    NSAttributedString.Key.foregroundColor: CountryManager.shared.config.countryNameTextColor])
        attributedText.append(NSAttributedString(string: language.nativeName,
                                                 attributes: [NSAttributedString.Key.font: CountryManager.shared.config.countryDescriptionTextFont,
                                                              NSAttributedString.Key.foregroundColor: CountryManager.shared.config.countryNameTextColor]))
        ...
        countryNameLabel.attributedText = attributedText
    }
}
