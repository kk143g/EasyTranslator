...
public final class CountryPickerViewController: UIViewController {
    ...
    override public func viewDidLoad() {
        ...
        // Exclude english, and it's by default downloaded
        languages = CountryManager.shared.getLanguages().filter({ $0.isoCode != "en" })
    ...
    func filter(for query: String?) {
        if query?.count == 0 {
            if CountryManager.shared.config.shouldShowLanguages {
                filteredLanguages = languages
            } else {
                filteredCountries = countries
            }
            ...
            if CountryManager.shared.config.shouldShowLanguages {
                filteredLanguages = languages
                    .filter { $0.name.lowercased().contains(text.lowercased()) || $0.nativeName.lowercased().contains(text.lowercased()) }
            } else {
                filteredCountries = countries
...
extension CountryPickerViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if CountryManager.shared.config.shouldShowLanguages {
            self.delegate?.languagePicker(didSelect: self.filteredLanguages[indexPath.row])
        } else {
